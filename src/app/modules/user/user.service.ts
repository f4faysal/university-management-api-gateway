...
const createStudent = async (req: Request) => {
  ...
  const uploadedImage: IClooudinaryResponse = await FileUploadHelper.uploadToCloudinary(file);

  if (uploadedImage) {
    req.body.profileImage = uploadedImage.secure_url;
  }

  const { academicDepartment } = req.body.student;
  ...
  const { academicSemester } = req.body.student;
  ...
  const { academicFaculty } = req.body.student;
  const academicFacultysResponse = await AuthService.get(
    `/academic-faculties?syncId=${academicFaculty}`
  );
  if (academicFacultysResponse.data && Array.isArray(academicFacultysResponse.data)) {
    req.body.student.academicFaculty = academicFacultysResponse.data[0].id;
  }
...
const createFaculty = async (req: Request) => {
  const file = req.file as IUploadFile;
  const uploadedProfileImage: IClooudinaryResponse = await FileUploadHelper.uploadToCloudinary(
    file
  );
  ...
  const { academicDepartment } = req.body.faculty;
  ...
  const { academicFaculty } = req.body.faculty;
  const academicFacultysResponse = await AuthService.get(
    `/academic-faculties?syncId=${academicFaculty}`
  );
  if (academicFacultysResponse.data && Array.isArray(academicFacultysResponse.data)) {
    req.body.faculty.academicFaculty = academicFacultysResponse.data[0].id;
  }
  // console.log(academicDepartmentResponse ,academicFacultysResponse )
